...
class MLP(object):
	...
	def _hidden_layers(self, inputs, training):
        ...
        # a stands for input, b stands for layer update
		a = inputs 

		layer_count = self.num_hid_layers

		for k in range(layer_count):
		    b = tf.layers.dense(a, self.num_hid_units, activation=tf.nn.sigmoid)
		    a = b
	...
	def _output_layer(self, inputs, training):
		...
		outputs = tf.layers.dense(inputs, self.num_hid_units, activation=tf.nn.tanh)

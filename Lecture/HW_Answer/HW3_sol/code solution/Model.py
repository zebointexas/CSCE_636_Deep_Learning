	...
	def train(self, x_train, y_train, max_epoch):
		...
		for epoch in range(1, max_epoch+1):
			...
			for i in range(num_batches):
				...
				x_batch = []
				for batch_i in range(self.conf.batch_size*i, self.conf.batch_size*(i+1)):
					x_batch.append(parse_record(curr_x_train[batch_i], True))
				x_batch = np.array(x_batch).reshape((self.conf.batch_size, 32, 32, 3))
				y_batch = curr_y_train[self.conf.batch_size*i:self.conf.batch_size*(i+1)]
				### END CODE HERE
	...
	def test_or_validate(self, x, y, checkpoint_num_list):
		...
		for checkpoint_num in checkpoint_num_list:
			...
			for i in tqdm(range(x.shape[0])):
				### YOUR CODE HERE
				feed_dict = {self.inputs: parse_record(x[i], False).reshape((1, 32, 32, 3)),
							self.labels: y[i]}

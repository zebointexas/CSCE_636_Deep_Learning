...
class PCA():
    ...
    def _do_pca(self):
        ...
        '''
        ### YOUR CODE HERE
        X_centered = self.X - np.expand_dims(np.mean(self.X, 1), 1)
    ...
    def reconstruction(self, Xp):
        ...
        '''
        ### YOUR CODE HERE
        X_re = np.dot(self.Up, self.Xp) + np.expand_dims(np.mean(self.X, 1), 1)
        ### END YOUR CODE
        return X_re
...
class AE():
    ...
    def _network(self, X):
        ...
        self.w = tf.Variable(initializer([self.n_features, self.d_hidden_rep]))

        hidden_rep = tf.matmul(tf.transpose(self.w), X)
        self.out_layer = tf.matmul(self.w, hidden_rep)


        # Note: here for the network without weights sharing
        self.w = tf.Variable(initializer([self.n_features, self.d_hidden_rep]))
        self.w2 = tf.Variable(initializer([self.n_features, self.d_hidden_rep]))

        hidden_rep = tf.matmul(tf.transpose(self.w), X)
        self.out_layer = tf.matmul(self.w2, hidden_rep)


        # Note: here for the network with more layers and nonlinear functions
        self.w = tf.Variable(initializer([self.n_features, self.d_hidden_rep*2]))
        self.w2 = tf.Variable(initializer([self.d_hidden_rep, self.d_hidden_rep*2]))


        self.w2_r = tf.Variable(initializer([self.d_hidden_rep*2, self.d_hidden_rep]))
        self.w_r = tf.Variable(initializer([self.n_features, self.d_hidden_rep*2]))


        hidden_rep = tf.nn.tanh(tf.matmul(tf.transpose(self.w), X))
        hidden_rep = tf.matmul(self.w2, hidden_rep)

        self.out_layer = tf.nn.tanh(tf.matmul(self.w2_r, hidden_rep))
        self.out_layer = tf.nn.relu(tf.matmul(self.w_r, self.out_layer))
        #
        #
        # ### END YOUR CODE
    ...
    def reconstruction(self, X):
        ...
        for i in range(n_samples): 
            ...
            if i==0:
                ...
                X_re = np.concatenate([X_re, curr_recons], 1)
